...
private:
  ...
  bool compareDet(const edm::DetSet<SiStripDigi>& detA, const edm::DetSet<SiStripDigi>& detB, const edm::Event& evt ) ;
...
bool SiStripDigiDiff::compareDet(const edm::DetSet<SiStripDigi>& detA, const edm::DetSet<SiStripDigi>& detB, const edm::Event& evt ) 
{
  bool hasDiff{false};
  if ( detB.size() != detA.size() ) {
    ...
    if (abs(detA.size()-detB.size())>100){
       edm::LogWarning("SiStripDigiDiff") << "Abnormal difference => detId : "<<detA.id<<"/"<<detB.id; 
    }
    hasDiff = true;
  } else {
  ...
  for ( const auto& dsetA : *digisA ) {
    ...
    if ( digisB->end() == i_dsetB ) {
      ...
      if ( m_ignoreAllZeros ) {
        ...
        areEqual = compareDet(dsetA_noz, dsetB_noz, evt);
      } else {
        areEqual = compareDet(dsetA, dsetB, evt);
      }
      if ( areEqual ) {
        ++goodMods;
      } else {
